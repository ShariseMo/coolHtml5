		...
		<table class="upload_table" border="1">
				...
				<tr>
					<td rowspan="3" class="temp_pic_box">
						<img src="/images/client_background/adv_templ_poster.png" class="template_pic" alt="">
						<h2>海报为主模板</h2>
					...
					<td class="second_td">
					<p>位置一(海报)</p>
					<input type="file" id="file_1" multiple id="file_3" multiple @change="add_poster">
				...
				<tr>
					<td class="second_td">
					<p>位置三(产品图)</p>
					<input type="file" id="file_3" multiple @change="add_pic">
			...
			uploadQiniu:function(file){
				...
				self.$http.post("http://upload.qiniu.com",fileData,{upload:progress}).then(
					function(response){
						...
						this.$http.post("/material",formData).then(
							function(response){
								console.log(response)
								self.showVideo = response.data.data;
							}
						)
					}
				)
